...
///
/// Each element contains two components, a (typically) 32x32x3 RGB image and a one-hot label encoding the category out
/// of 10
pub struct Cifar10 {
	images: Vec<Vec<u8>>,
	labels: Vec<u8>,
...
impl DataSet for Cifar10 {
	fn get(&mut self, i: usize) -> Vec<ArcArray<f32, IxDyn>> {
		...
		for i in 0..1024 {
			image_vec[i * 3] = f32::from(bytes[i]) / 255.0;
			image_vec[i * 3 + 1] = f32::from(bytes[i + 1024]) / 255.0;
			image_vec[i * 3 + 2] = f32::from(bytes[i + 2048]) / 255.0;
		}
		let image = ArcArray::from_shape_vec(&[32, 32, 3][..], image_vec).unwrap();
...
impl DataSet for Cifar100 {
	fn get(&mut self, i: usize) -> Vec<ArcArray<f32, IxDyn>> {
		...
		let image = ArcArray::from_shape_vec(&[32, 32, 3][..], image_vec).unwrap();
		let mut coarse_one_hot_label = ArcArray::zeros(IxDyn(&[20]));
		coarse_one_hot_label[self.coarse_labels[i] as usize] = 1.0;
		let mut fine_one_hot_label = ArcArray::zeros(IxDyn(&[100]));
	...
	fn components(&self) -> Vec<String> {
		vec![
			"Images".to_string(),
			"Coarse Labels".to_string(),
			"Fine Labels".to_string(),
		]
	}
}
...
fn read_cifar100_file(mut file: File) -> (Vec<u8>, Vec<u8>, Vec<Vec<u8>>) {
	...
	let mut images = Vec::with_capacity(n);
	for _ in 0..n {
		coarse_labels.push(buf[i]);
		i += 1;
		fine_labels.push(buf[i]);
		i += 1;

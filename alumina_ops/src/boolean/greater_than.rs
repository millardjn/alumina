...
pub fn greater_than<I1, I2>(input1: I1, input2: I2) -> Result<Node, OpBuildError>
where
	I1: Into<Node>,
	I2: Into<Node>,
{
	...
	let output = input1
		...
		.set_name_unique(&format!("greater_than({},{})", input1, input2));
	let _op = GreaterThan::new_default(input1, input2, output.clone())
		.build()
		.expect("Error building GreaterThan Op");
	Ok(output)
}

pub type GreaterThan = BinaryElementwise<GreaterThanFunc>;

#[derive(Clone, Debug, Default)]
pub struct GreaterThanFunc {}

impl BinaryFunc for GreaterThanFunc {
	#[inline]
...
mod tests {
	...
	fn forward_test() {
		...
		let output = greater_than(&input1, &input2).unwrap();

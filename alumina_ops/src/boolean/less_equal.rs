...
pub fn less_equal<I1, I2>(input1: I1, input2: I2) -> Result<Node, OpBuildError>
where
	I1: Into<Node>,
	I2: Into<Node>,
{
	...
	let output = input1
		...
		.set_name_unique(&format!("less_equal({},{})", input1, input2));
	let _op = LessEqual::new_default(input1, input2, output.clone())
		.build()
		.expect("Error building LessEqual Op");
	Ok(output)
}

pub type LessEqual = BinaryElementwise<LessEqualFunc>;

#[derive(Clone, Debug, Default)]
pub struct LessEqualFunc {}

impl BinaryFunc for LessEqualFunc {
	#[inline]
...
mod tests {
	...
	fn forward_test() {
		...
		let output = less_equal(&input1, &input2).unwrap();

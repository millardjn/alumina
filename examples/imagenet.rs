...
fn main(){
	...
	if args.len() <= 1 {
		...
		let mut input = NodeData::new_blank(DataShape::new(CHANNELS, vec![image.dimensions().0 as usize, image.dimensions().1 as usize], 1));
		...
		let activ_func: Box<Fn(&(NodeIndex, NodeShape), &(NodeIndex, NodeShape)) -> Box<Operation>>= Box::new(|input, output| BeLU::new(input, output, ParamSharing::Spatial, "generic activ", BeLU::init_porque_no_los_dos()));
...
fn upscaler_train(){
	...
	//let mut training_set = ImagenetSupplier::new(Path::new("D:/ML/ImageNet"), 120, 120, Cropping::Random);
	...
	{ // Add occasional test set evaluation as solver callback
		...
		let mut test_set = ImageFolderSupplier::new(Path::new("C:/Set14"), None);
...
fn upscaler_net6(factor: usize, training: bool, regularisation: f32, activation_func: &Fn(&(NodeIndex, NodeShape), &(NodeIndex, NodeShape)) -> Box<Operation>) -> Graph {
	let mut g = Graph::new();
	...
	ops.push(Convolution::new(&input, &l1_conv, vec![5, 5], Padding::Same, "conv1", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&input, &l2_conv, vec![5, 5], Padding::Same, "conv2", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&input, &l3_conv, vec![5, 5], Padding::Same, "conv3", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&input, &expand, vec![1, 1], Padding::Same, "conv4", Convolution::init_msra(0.01)));

	ops.push(Convolution::new(&l1_activ, &l2_conv, vec![3, 3], Padding::Same, "conv5", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&l1_activ, &l3_conv, vec![3, 3], Padding::Same, "conv6", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&l1_activ, &expand, vec![1, 1], Padding::Same, "conv7", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&l2_activ, &l3_conv, vec![3, 3], Padding::Same, "conv8", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&l2_activ, &expand, vec![1, 1], Padding::Same, "conv9", Convolution::init_msra(0.1)));
	ops.push(Convolution::new(&l3_activ, &expand, vec![1, 1], Padding::Same, "conv10", Convolution::init_msra(0.1)));


	// -- Output
	ops.push(Bias::new(&expand, ParamSharing::Spatial, "expand_bias", init_fill(0.0)));
	ops.push(activation_func(&expand, &expand_activ));
	ops.push(Expand::new(&expand_activ, &expand_out, vec![factor, factor], "expand"));
	ops.push(Convolution::new(&expand_out, &output, vec![5, 5], Padding::Same, "conv_out", Convolution::init_msra(0.1)));
	...
	if training {
		...
		//g.add_operation(Pooling::new(&input_hr, &input, vec![factor, factor], "input_pooling"));
		...
		g.add_operation(ShapeConstraint::new(&input_hr, &expand_out, vec![Arc::new(|d| d), Arc::new(|d| d)], "expand_shape"));	
	} else {
		g.add_operation(ShapeConstraint::new(&input, &output, vec![Arc::new(move|d| d*factor), Arc::new(move|d| d*factor)], "output_shape"));	
		g.add_operation(ShapeConstraint::new(&input, &expand_out, vec![Arc::new(move|d| d*factor), Arc::new(move|d| d*factor)], "expand_shape"));
	}
...
fn upscaler_net(factor: usize, training: bool, regularisation: f32, activation_func: &Fn(&(NodeIndex, NodeShape), &(NodeIndex, NodeShape)) -> Box<Operation>) -> Graph {
	let mut g = Graph::new();
	...
	//g.add_operation(Convolution::new(&f_activ, &expand, vec![1, 1], Padding::Same, "conv1", Convolution::init_msra(0.1)));
	...
	for _ in 0..1 {
		...
		ops.push(Convolution::new(&l1_activ, &expand, vec![3, 3], Padding::Same, "conv8", Convolution::init_msra(0.1)));

		ops.push(Convolution::new(&l2_activ, &l3_conv, vec![3, 3], Padding::Same, "conv9", Convolution::init_msra(1.0)));
		ops.push(Convolution::new(&l2_activ, &expand, vec![3, 3], Padding::Same, "conv11", Convolution::init_msra(0.1)));

		ops.push(Convolution::new(&l3_activ, &expand, vec![3, 3], Padding::Same, "conv13", Convolution::init_msra(0.1)));
...
fn upscaler_net4(factor: usize, training_net: bool, activation_func: &Fn(&(NodeIndex, NodeShape), &(NodeIndex, NodeShape)) -> Box<Operation>) -> Graph {
	let mut g = Graph::new();
	...
	g.add_operation(LinearInterp::new(&input_lin, &upscale_lin, vec![factor, factor], "linterp"));
	...
	g.add_operation(Expand::new(&exit_activ, &exit_expand, vec![factor, factor], "exit_expand"));
	

	g.add_operation(Convolution::new(&exit_expand, &output, vec![5, 5], Padding::Same, "output_conv", Convolution::init_msra(1.0)));
	...
	// g.add_operation(Expand::new(&exit_conv, &output, vec![factor, factor], "exit_expand"));
	// g.add_operation(Bias::new(&output, ParamSharing::Spatial, "output_bias", init_fill(0.0)));
	...
	if training_net {
		...
		g.add_operation(ShapeConstraint::new(&input_hr, &exit_expand, vec![Arc::new(|d| d), Arc::new(|d| d)], "output_shape"));	
		g.add_operation(ShapeConstraint::new(&input_hr, &upscale_lin, vec![Arc::new(|d| d), Arc::new(|d| d)], "output_shape"));	
	} else {
		g.add_operation(ShapeConstraint::new(&input, &exit_expand, vec![Arc::new(move|d| d*factor), Arc::new(move|d| d*factor)], "output_shape"));
		g.add_operation(ShapeConstraint::new(&input, &upscale_lin, vec![Arc::new(move|d| d*factor), Arc::new(move|d| d*factor)], "output_shape"));		
	}
	...
	g.add_operation(Convolution::new(&entry, &exit, vec![5, 5], Padding::Same, "cap_conv", Convolution::init_msra(1.0)));

	g
}

fn add_single_layer(g: &mut Graph, entry: &(NodeIndex, NodeShape), exit: &(NodeIndex, NodeShape),
	up_channels: usize, down_channels: usize, factor: usize, up_conv_size: usize,
	activation_func: &Fn(&(NodeIndex, NodeShape), &(NodeIndex, NodeShape)) -> Box<Operation>
	) -> ((NodeIndex, NodeShape), (NodeIndex, NodeShape)){
	...
	g.add_operation(Convolution::new(&entry, &up_conv, vec![up_conv_size, up_conv_size], Padding::Same, "up_conv", Convolution::init_msra(1.0)));
	...
	let down_expand = if factor == 1 {
		...
		g.add_operation(Expand::new(&down_activ, &down_expand, vec![factor, factor], "down_expand"));
		down_expand
	};
	g.add_operation(Convolution::new(&down_expand, &exit, vec![3, 3], Padding::Same, "down_conv", Convolution::init_msra(1.0)));
	...
	g.add_operation(Convolution::new(&up_activ, &cross_conv, vec![3, 3], Padding::Same, "cross_conv", Convolution::init_msra(0.1)));
	...
	let up_pool = if factor == 1 {
		...
		g.add_operation(Pooling::new(&up_activ, &up_pool, vec![factor, factor], "up_pool"));
		up_pool
	};
...
fn upscaler_linear_test(){
	...
	let mut training_set = ImagenetSupplier::new(Path::new("D:/ML/ImageNet"), 227, 227, Cropping::Centre);
	...
	{ // Add occasional test set evaluation as solver callback
		...
		let mut test_set = ImageFolderSupplier::new(Path::new("D:/ML/Set14"), None);
		let n = test_set.epoch_size() as usize;
...
fn upscaler_linear_net(factor: usize) -> Graph{
	...
	let ops: Vec<Box<Operation>> = vec![
		...
		Pooling::new(&input_linear, &input_pool, vec![factor, factor], "pooling1"),
		LinearToSrgb::new(&input_pool, &input_lr, "lin2srgb"),

		// Upscale
		Convolution::new(&input_lr, &conv1, vec![3, 3], Padding::Same, "conv1", init_fill(1.0/27.0)),
		Expand::new(&conv1, &output, vec![factor, factor], "expand1"),
		ShapeConstraint::new(&input, &output, vec![Arc::new(|d| d), Arc::new(|d| d)], "shape1"),
...
fn data_to_img(image_node: NodeData) -> DynamicImage {
...
fn img_to_data(data: &mut[f32], image: &DynamicImage){

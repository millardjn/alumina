...
impl Op for Mse {
	...
	fn build(self, graph: &mut GraphDef, op_id: &OpID) -> Result<Self::InstanceType> {
		...
		let pass_id = graph.add_pass(MsePass{input1_id: self.input1.clone(), input2_id: self.input2.clone(), multiplier: self.multiplier});
...
impl OpInstance for MseInstance {
	fn type_name(&self) -> &'static str {"Mse"}

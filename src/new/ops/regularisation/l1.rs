...
/// An `Op` which implements a loss equal to the L1 norm
///
/// By default this `Op` has no output and will generate loss and gradients.
///
/// If `output()` is set, the L1 loss will be written to that Node,
	...
	fn build(self, graph: &mut GraphDef, _op_id: &OpID) -> Result<Self::InstanceType> {
		...
		let loss_type = if let Some(output_id) = self.output {
			LossType::Output{
				...
				forward_id: graph.add_pass(L1Forward::new(
					self.multiplier,
					self.input_id.clone(),
					output_id.clone(),
					self.mean_axes.clone(),
					self.keep_dims)),
				backward_id: graph.add_pass(L1Backward::new(
					self.multiplier,
					self.input_id.clone(),
					output_id.clone(),
					self.mean_axes.clone(),
			...
			LossType::Joint{
				pass_id: graph.add_pass(L1JointPass::new(
					self.multiplier,
					self.input_id.clone(),
					self.mean_axes.clone()))
			}
...
impl OpInstance for L1Instance {
	...
	fn propagate_shape_constraints(&self, shapes: &mut GraphShapes) -> Result<()>{
		if let &LossType::Output{ref output_id, ..} = &self.loss_type {
			...
			let output_shape: NodeShape = calc_output_shape(input_shape.slice(), &self.mean_axes, self.keep_dims).into();
...
impl Pass for L1JointPass {
	fn type_name(&self) -> &'static str {"L1JointPass"}
	...
	fn run (&self, data: &Storage) -> Result<Box<Any>>{
		...
		for (input_chunk, mut input_grad_chunk) in iter1.zip(iter2) {
			...
			.apply(|input, input1_grad| {
				error += input.abs()*multiplier;
				*input1_grad += input.signum()*multiplier;
			});
		}
...
impl Pass for L1Forward {
	fn type_name(&self) -> &'static str {"L1Forward"}
...
impl Pass for L1Backward {
	fn type_name(&self) -> &'static str {"L1Backward"}
	...
	fn run (&self, data: &Storage) -> Result<Box<Any>>{
		...
		for (input_chunk, mut input_grad_chunk) in iter1.zip(iter2) {
			...
			.apply(|output_grad, input, input_grad| { 
				*input_grad +=  input.signum()*multiplier*output_grad;
			});
		}
...
fn _l1_backprop() -> Result<()>{
	...
	let _o1 = g.new_op(L1::new(&node1), tag![])?;
...
#[test]
fn test_l1_output_backprop(){
	_l1_output_backprop().unwrap();
}
...
fn _l1_output_backprop() -> Result<()>{
	...
	let _o1 = g.new_op(L1::new(&node1).mean_axes(&[0, -1]).output(&node2), tag![])?;

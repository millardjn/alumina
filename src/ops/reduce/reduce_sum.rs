...
impl Op for ReduceSum {
	...
	fn build(self, graph: &mut GraphDef, _op_id: &OpID) -> Result<Self::InstanceType> {
		...
		Ok(ReduceSumInstance{
			...
			backward_id:graph.add_pass(ReduceSumBackward::new(
				self.input_id.clone(),
				self.output_id.clone(),
				self.axes.clone(),
				self.keep_dims,
...
impl Pass for ReduceSumBackward {
	fn type_name(&self) -> &'static str {"ReduceSumBackward"}
...
#[test]
fn test_reduce_sum_backprop(){
	_reduce_sum_backprop().unwrap();
}

fn _reduce_sum_backprop() -> Result<()>{
	use graph::GraphDef;
	...
	let _o1 = g.new_op(ReduceSum::new(&node1, &node2).axes(&[-2, 1]), tag![])?;
...
#[test]
fn test_reduce_sum_keep_dims_backprop(){
	_reduce_sum_keep_dims_backprop().unwrap();
}

fn _reduce_sum_keep_dims_backprop() -> Result<()>{
	use graph::GraphDef;
	use ops::numeric_check::numeric_test;
	use ops::loss::mse::Mse;
	use ordermap::OrderMap;
	...
	let _o1 = g.new_op(ReduceSum::new(&node1, &node2).axes(&[-2, 1]).keep_dims(true), tag![])?;
	let _o2 = g.new_op(Mse::new(&node2, &node3), tag![])?;

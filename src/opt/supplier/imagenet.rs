...
pub struct ImagenetSupplier<S: Selector>{
	count: u64,
	epoch: usize,
		...
		match self.crop {
			...
			Cropping::Centre {width, height}| Cropping::Random{width, height} => {
				...
				for (data, label) in data.values.chunks_mut(data_len/n).zip(labels.values.chunks_mut(labels_len/n)){
					...
					load_crop(&mut self.orders[label_index], &self.paths[label_index], data, &self.crop);
				}
...
impl<S: Selector> ImagenetSupplier<S>{
		
	pub fn new(path: &Path, crop: Cropping) -> ImagenetSupplier<S> {
		...
		let orders = paths.iter().map(|vec| S::new(vec.len())).collect();
		let n = paths.len();
		ImagenetSupplier{
			count: 0,
